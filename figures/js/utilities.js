...
function calcStats(d) {
    ...
    let mean = d.reduce((a, b) => a + b)/d.length
    let std = Math.sqrt(d.map(e => (e - mean)*(e - mean)).reduce((a, b) => a + b)/(d.length - 1))

    let t
    if (d.length == 10) { t = 2.262 }
    else if (d.length == 50) { t = 2.010 }
    else { return {} }

    let interval = t*std/Math.sqrt(d.length)
    return { "mean": mean, "std": std, "interval": interval }
}
